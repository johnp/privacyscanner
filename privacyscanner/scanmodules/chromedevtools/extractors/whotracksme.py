# Note: this is a slightly modified version from Matthias Kraus' `whotracksme` branch that also
#       collects unrecognized fqdns for comparison with other sources of truth (e.g. disconnect.me).
#       and has some minor logic cleanup.
...
# TODO: why does this not work with geizhals.at ?
class WhotracksmeExtractor(Extractor):

    def extract_information(self):
        if 'third_parties' not in self.result:
            self.logger.error("Missing `third_parties`")
            return

        domaindb, companydb = self._load_tracker_db(True)
        third_party_domains = self.result['third_parties']['fqdns']

        if third_party_domains and len(third_party_domains) > 0:
            organizations = dict(domains={}, details={}, unrecognized=[])

            # TODO: figure out where empty third party fqdns / domains come from
            for element in filter(None, third_party_domains):
                scn = self._find_company_network(domaindb, element)
                if scn is not None:
                    organizations['domains'][element] = scn
                    organizations['details'][scn] = companydb[scn]
                else:
                    organizations['unrecognized'].append(element)

            organizations['unrecognized'].sort()
